								...
								<div class="col-md-4">
									<div class="thumbnail">
										...
										<div class="caption">
											...
											<p class="beginTime">考勤时间: 
												<c:if test="${ehp.beginTime == null }">暂未定义</c:if>
												<c:if test="${ehp.beginTime != null }">${ehp.beginTime }</c:if>
											</p>
											<p>
									 			<a class="btn btn-default btn-lg btn-block beginExam" href="../review?studentId=${sessionScope.loginStudent.studentId }&examPaperId=${ehp.examPaperId }&score=${ehp.examScore }&examPaperName=${ehp.examPaperName }">考勤结果</a>
											</p>
										</div>
									</div>
								</div>
