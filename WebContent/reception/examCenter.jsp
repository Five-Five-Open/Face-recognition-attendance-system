								...
								<div class="col-md-4">
									<div class="thumbnail">
										...
										<div class="caption">
											...
											<p>考勤科目: ${ep.course.courseName } &emsp;&emsp; 考勤时长: <span id="examPaperTime${ep.examPaper.examPaperId }">${ep.examPaper.examPaperTime }</span> 分钟</p>
											<p>
												考勤类型: 
												<c:if test="${ep.examPaper.examPaperEasy == 0 }">二维码考勤</c:if>
												<c:if test="${ep.examPaper.examPaperEasy == 1 }">GPS考勤</c:if>
												<c:if test="${ep.examPaper.examPaperEasy == 2 }">人脸识别考勤</c:if>
											</p>
											<p class="beginTime">开始时间: ${ep.beginTime }</p>
											<p>
												<a class="btn btn-default btn-lg btn-block beginExam" id="${ep.examPaper.examPaperId }" href="begin?classId=${ep.clazz.classId }&examPaperId=${ep.examPaper.examPaperId }&studentId=${sessionScope.loginStudent.studentId }&examTime=${ep.examPaper.examPaperTime }&gradeId=${gradeId }&beginTime=${ep.beginTime }">进入考勤</a>
											</p>
										</div>
									</div>
								</div>
